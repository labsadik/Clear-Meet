...
function MeetingSession({ meeting, participant, onLeave }: Omit<Props, "token" | "displayName">) {
  ...
  const { meetingState } = useMeeting({
    ...
    onError: (error) => {
      ...
      setJoinError(
        error instanceof Error ? error.message : "Could not join the meeting.",
      );
    },
  });

  useEffect(() => {
    if (meetingState === "JOINED" || meetingState === "LEFT") return;
  ...
  const load = async () => {
    const { data } = await supabase
      ...
      .select("id,meeting_id,user_id,role,status,joined_at,profiles(display_name,avatar_url)")
  ...
  const admit = async (id: string) => {
    try {
      await supabase.functions.invoke("admit-participant", {
        body: { meetingId: meeting.id, participantId: id },
      });
    } catch (error) {
      console.error(error);
    }
  ...
  const reject = async (id: string) => {
    try {
      await supabase.functions.invoke("reject-participant", {
        body: { meetingId: meeting.id, participantId: id },
      });
    } catch (error) {
      console.error(error);
        ...
        <main className="meeting-main">
          <div className="card narrow">
            <div className="eyebrow">
              {joinError ? "VIDEO ERROR" : meetingState === "JOINING" ? "CONNECTING" : "PREPARING"}
            </div>
            <h1>
              {joinError || joinTimedOut ? "Could not connect to meeting" : "Joining meeting…"}
            </h1>
            <p className="muted">
              {joinError || joinTimedOut
                ? joinError
                : "You can join without camera or microphone access. Turn them on after you enter."}
            </p>
      ...
      <main className="meeting-main">
        ...
        <div className="meeting-card">
          <div className="meeting-row">
            <h2>People</h2>
            <span className="badge">
              {people.filter((p) => p.status === "admitted").length} admitted
            </span>
          </div>

          {isHost &&
            people.some((p) => p.status === "waiting") && (
              <div style={{ marginTop: 14 }}>
                ...
                {people
                  ...
                  .map((p) => (
                    ...
                    >
                      <span>{p.profiles?.display_name || "Guest"}</span>
          ...
          <div className="actions" style={{ marginTop: 12 }}>
            ...
            <button className="button secondary" onClick={() => void send()}>
              Send
            </button>
          </div>
        </div>
