...
function MeetingSession({meeting,participant,onLeave}:{meeting:Meeting;participant:ParticipantRow;onLeave:()=>Promise<void>}) {
  ...
  useEffect(()=>{
    ...
    const channel=supabase.channel(`meeting-${meeting.id}`)
      ...
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"meeting_messages",filter:`meeting_id=eq.${meeting.id}`},payload=>setChat(v=>[...v,payload.new as MessageRow]))
      .subscribe();
  ...
  const copy=()=>{
    void navigator.clipboard.writeText(`${window.location.origin}/meeting/${meeting.public_slug}`).then(()=>{setCopied(true);window.setTimeout(()=>setCopied(false),1500);}).catch(console.error);
  };

  if(!joined) return <div className="meeting-page"><header className="meeting-header"><a href="/dashboard" className="brand"><span className="logo"><Video size={17}/></span><span>{meeting.title}</span></a></header><main className="meeting-main"><div className="card narrow"><div className="eyebrow">{joinStatus}</div><h1>{joinStatus==="ERROR"?"Could not join meeting":"Joining meeting…"}</h1><p className="muted">You can join without camera or microphone access. Turn them on after you enter.</p><button className="button secondary" style={{marginTop:14}} onClick={()=>void onLeave()}>Leave</button></div></main></div>;
...
function ParticipantTile({participantId}:{participantId:string}){
  ...
  useEffect(()=>{const video=videoRef.current;if(!video)return;if(webcamOn&&webcamStream){video.srcObject=new MediaStream([webcamStream.track]);void video.play().catch(()=>{});}else video.srcObject=null;return()=>{video.srcObject=null};},[webcamOn,webcamStream]);
  useEffect(()=>{const audio=audioRef.current;if(!audio)return;if(micOn&&micStream){audio.srcObject=new MediaStream([micStream.track]);void audio.play().catch(()=>{});}else audio.srcObject=null;return()=>{audio.srcObject=null};},[micOn,micStream]);
  return <div className="video-tile">{webcamOn?<video ref={videoRef} autoPlay playsInline muted={isLocal}/>:<div className="avatar-fallback">{(displayName||"?").slice(0,1).toUpperCase()}</div>}<audio ref={audioRef} autoPlay playsInline muted={isLocal}/><span className="tile-label">{displayName||"Participant"}{micOn?"":" · muted"}</span></div>;
}
...
function MeetingControls({joined,onLeave}:{joined:boolean;onLeave:()=>Promise<void>}){
  ...
  const action=async(fn:()=>unknown)=>{if(!joined||busy)return;setBusy(true);try{await Promise.resolve(fn());}catch(error){console.error("VideoSDK media action failed",error);}finally{setBusy(false);}};
  return <div className="control-bar"><button disabled={!joined||busy} className={`control ${micOn?"active":""}`} title={joined?"Microphone":"Joining meeting…"} onClick={()=>void action(()=>toggleMic())}>{micOn?<Mic size={19}/>:<MicOff size={19}/>}</button><button disabled={!joined||busy} className={`control ${webcamOn?"active":""}`} title={joined?"Camera":"Joining meeting…"} onClick={()=>void action(()=>toggleWebcam())}>{webcamOn?<Camera size={19}/>:<CameraOff size={19}/>}</button><button disabled={!joined||busy} className={`control ${sharing?"active":""}`} title={joined?"Share screen":"Joining meeting…"} onClick={()=>void action(async()=>{await enableScreenShare();setSharing(true);})}><MonitorUp size={19}/></button><button className="control leave" title="Leave" onClick={()=>void onLeave()}><PhoneOff size={19}/></button></div>;
}
